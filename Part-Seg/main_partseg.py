...
def train(args, io):
    ...
    for epoch in range(args.epochs):
        ...
        for data, label, seg in train_loader:
            ...
            opt.zero_grad()
            seg_pred = model(data, label_one_hot, k=50)
        ...
        for data, label, seg in test_loader:
            ...
            seg_pred = 0
            #GMPF Testing Strategy
            for k in [41,48,50,54,60,65]:
               ls = model(data, label_one_hot, k)
               #ls = F.softmax(ls,dim=-1)
               if k == 41 or k == 48 or k == 50 or k == 54 or k == 60:
                a = 2
               else:
                a = 1
               seg_pred += a*ls 
        ...
        if np.mean(test_ious) >= best_test_iou:
            ...
            torch.save(model.state_dict(), 'outputs/' + args.exp_name + '/models/model.t7')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='DuPMAM', metavar='N',
                        choices=['DuPMAM'],
                        help='Model to use, [DuPMAM]')
    parser.add_argument('--dataset', type=str, default='shapenetpart', metavar='N',
                        choices=['shapenetpart'])
    parser.add_argument('--class_choice', type=str, default= 'table', metavar='N',
